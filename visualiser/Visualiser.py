...
class Visualiser:
    ...
    def _draw_scene(self):
        ...
        self._ax_plot.set_xlim(left=self._left_lim, right=self._right_lim)
        self._ax_plot.set_ylim(bottom=self._bottom_lim, top=self._top_lim)

        container = self._scenes[self._active_scene]

        for rect in container.rects:
            self._ax_plot.add_patch(copy(rect))

        # recheck
        for lcoll in container.lines_collections:
            for line in lcoll.lines:
                self._ax_plot.plot(line[:, 0], line[:, 1], **lcoll.kwargs, zorder=1)

        for pcoll in container.points_collections:
            self._ax_plot.scatter(pcoll.points[:, 0], pcoll.points[:, 1], **pcoll.kwargs, zorder=2)
    ...
    def draw(self):
        ...
        self._ax_plot = plt.axes((0.05, 0.2, 0.9, 0.7))
